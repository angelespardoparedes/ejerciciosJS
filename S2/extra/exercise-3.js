...
const vecesSonidosFavoritos={}; 
for(const user of users){
    const userFavouriteSounds= user.favoritesSounds; 
    //Vemos si el sonido esta en el objeto de conteo
    for(const sonido in userFavouriteSounds){
        //Si está incrementamos el conteo
        if(vecesSonidosFavoritos[sonido]){
            vecesSonidosFavoritos[sonido]++; 
            //Si no, inicializamos a 1
        }else{
            vecesSonidosFavoritos[sonido]=1; 
        }
        
    }
}

for(const sonido in vecesSonidosFavoritos){
    console.log(`${sonido}: ${vecesSonidosFavoritos[sonido]} veces`); 
}
